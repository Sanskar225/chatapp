...
const ChatContainer = ({ selectedUser, setSelectedUser }) => {
  ...
  }, [messagesDummyData]) // Runs when messages change
        ...
        {messagesDummyData.map((message, index) => (
          <div
            ...
            className={`flex items-end gap-2 ${
              message.senderId !== '680f50e4f3cd28382ecf9'
                ? 'flex-row-reverse'
                : 'justify-end'
            }`}
          >
            {message.image ? (
              <img src={assets.image} alt='' className='max-w-[200px] rounded-lg' />
            ) : (
              <p
            ...
            <div className='flex flex-col items-center'>
              ...
              />
              <p className='text-gray-500 text-xs'>{formatMessageTime(message.createdAt)}</p>
            </div>
          </div>
        ))}
